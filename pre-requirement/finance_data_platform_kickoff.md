...
> 금융 데이터를 Snowflake 기반으로 수집·적재·모델링하고, Power BI 대시보드와 자연어 질의 AI를 제공하는 End-to-End 데이터 분석 플랫폼
...
| 주요 사용자 | 본인 (학습용) |
| 데이터 소스 | yfinance (주가), FRED API (경제지표), Alpha Vantage (기술지표), 공공데이터포털/KRX (한국 주가) — 전부 무료 |
...
| 데이터 웨어하우스 | Snowflake | 클라우드 DW 표준, 무료 트라이얼, Star Schema 지원 |
| 데이터 모델링 | Star Schema (Fact/Dim/Mart) | 분석 최적화, 업계 표준 패턴 |
| 시각화 | Power BI | 대시보드 표준, Snowflake 네이티브 커넥터 |
| AI 질의 (v2) | Claude/OpenAI API (Text-to-SQL) | 자연어 → SQL 변환, 결과 해석 (평가 후 조정됨) |
| 오케스트레이션 | Python 스크립트 (배치) | 학습용, 단순 구조 |
...
| 환경 | Snowflake 무료 트라이얼 (크레딧 한도 제약) |
...
```
[API 소스] → [Python 수집] → [Snowflake raw_data] → [Star Schema 변환] → [Mart View] → [Power BI / AI 질의]
```

| 단계 | 설명 | 도구/서비스 |
|---|---|---|
| 1 | 외부 API에서 금융 데이터 수집 | Python (yfinance, fredapi, requests) |
| 2 | raw_data 스키마에 원본 적재 | Snowflake Connector for Python |
| 3 | Star Schema로 Fact/Dim 테이블 변환 | Snowflake SQL (CREATE TABLE AS SELECT) |
| 4 | 부서별 Mart View 생성 | Snowflake SQL (CREATE VIEW) |
| 5 | 대시보드 시각화 | Power BI + Snowflake 커넥터 |
| 6 | 자연어 질의 → SQL 생성 → 실행 → 해석 (v2) | Claude/OpenAI API + Python (평가 후 조정됨) |
...
| 컬럼 | 타입 | 설명 |
|---|---|---|
| date_key | INT | dim_date FK |
| symbol_key | INT | dim_symbol FK |
...
|---|---|---|
| date_key | INT | dim_date FK |
| indicator_key | INT | dim_indicator FK |
| value | DECIMAL | 지표 값 |
...
| indicator_code | VARCHAR | 지표 코드 (예: FEDFUNDS) |
| indicator_name | VARCHAR | 지표명 |
| source | VARCHAR | 출처 (FRED/Alpha Vantage) |
| unit | VARCHAR | 단위 (%, 포인트 등) |
...
| 3 | Snowflake 연결 끊김 (크레딧 소진) | 높 | 크레딧 모니터링 + 조기 경고 |
...
### 환경 구성
- [ ] Python 3.x 설치 확인
- [ ] 필수 패키지 설치 가능 확인 (yfinance, fredapi, requests, snowflake-connector-python)
...
- [ ] Snowflake 연결 테스트 (Python Connector)
- [ ] Snowflake 크레딧 한도 및 XS warehouse 설정 확인
- [ ] Power BI에서 Snowflake 커넥터 연결 테스트
...
| 3 | 시장 유효성 | 🟢 | Snowflake + Star Schema + Power BI는 6개월 후에도 현역 기술 |
...
**현재 상태**: Snowflake + Star Schema + Power BI는 데이터 엔지니어링 학습에서 흔한 조합
...
### 기능 동작 기준
- [ ] yfinance, FRED API에서 데이터를 수집하여 Snowflake raw_data에 적재가 완료된다
- [ ] raw_data에서 Star Schema(Fact 2, Dim 3)로 변환이 에러 없이 완료된다
...
- [ ] Snowflake 적재 건수가 API 수집 건수와 일치한다
...
| FR-004 | Snowflake 적재 | 수집된 원본 데이터를 Snowflake raw_data 스키마에 적재한다 | 필수 | 기능#2 |
| FR-005 | Star Schema 변환 | raw_data에서 Fact 2개(daily_price, economic_indicator) + Dim 3개(date, symbol, indicator)로 변환한다 | 필수 | 기능#3 |
...
| FR-008 | 자연어 질의 AI | 자연어 입력을 SQL로 변환하여 Snowflake에서 실행하고 결과를 해석한다 | 선택 (v2) | 기능#5 |
...
| NFR-002 | 성능 | Mart View 쿼리 응답 | 단일 쿼리 10초 이내 (XS warehouse) | 중간 |
...
| NFR-007 | 데이터 품질 | 완전성 | 필수 필드(date, ticker, close_price) 누락률 0% | 높음 |
| NFR-008 | 데이터 품질 | 일관성 | Snowflake 적재 건수 = API 수집 건수 (불일치 시 경고) | 높음 |
...
5. Snowflake raw_data 스키마에 원본 데이터를 적재한다
6. 적재 건수를 로그에 기록한다
...
1. 사용자가 Power BI를 열고 Snowflake에 연결한다
...
- **Snowflake 크레딧 부족 시**: 수집까지만 실행하고 적재는 크레딧 충전 후 재시도

#### 예외 흐름

- **API 호출 실패 (Rate Limit/서버 다운)**: 에러 로그 기록, 해당 API 건너뜀, 수동 재실행 안내
- **데이터 품질 검증 실패**: 경고 로그 기록, 적재는 계속 진행
- **Snowflake 연결 실패**: 에러 로그 기록, 수집 데이터는 로컬 CSV로 임시 저장 안내

### 9-4. 제약사항 보강

| 유형 | 내용 | 근거 |
|---|---|---|
| 비용 | 무료 API만 사용, Snowflake 무료 트라이얼 크레딧 범위 내 운영 | 섹션 1 제약사항 |
...
| 데이터 규모 | 100+ 종목 × 5년+ (수만~수십만 건) — XS warehouse로 처리 가능 범위 | 인터뷰 결과 |
...
```
[External APIs]           [Python Application]              [Snowflake DW]            [Serving]

┌─────────────┐     ┌───────────────────────────┐
│  yfinance   │◄────│                           │
│  FRED API   │◄────│    main.py (Orchestrator)  │
│  Alpha Vant.│◄────│                           │
│  KRX       │◄────│  ┌─────────┐ ┌─────────┐  │     ┌──────────────┐
└─────────────┘     │  │Collector│→│Validator │  │     │              │
                    │  └────┬────┘ └────┬────┘  │     │  raw_data    │
...
| Collector | 외부 API에서 금융 데이터 수집 | Python (yfinance, fredapi, requests) | API 엔드포인트, 종목 목록 | DataFrame (원본 데이터) |
| Validator | 수집 데이터 품질 검증 (결측/이상치/중복) | Python (pandas) | DataFrame | 검증 결과 (pass/warn) + 로그 |
| Loader | Snowflake raw_data 스키마에 Full Refresh 적재 | snowflake-connector-python | DataFrame | raw_data 테이블 |
| Transformer | raw_data → Star Schema 변환 (Snowflake 내부) | Snowflake SQL (CTAS) | raw_data 테이블 | Fact 2 + Dim 3 테이블 |
| Mart Manager | 분석용 Mart View 생성/갱신 | Snowflake SQL (CREATE VIEW) | Fact/Dim 테이블 | Mart View 3개 |
| Pipeline Logger | 각 단계 성공/실패/건수 기록 | Python (logging) | 각 단계 상태 | 구조화된 로그 파일 |
| Power BI Dashboard | Mart View 데이터 시각화 | Power BI + Snowflake Connector | Mart View | 대시보드 차트 |
...
| 소스 | 대상 | 방식 | 프로토콜 |
|---|---|---|---|
| Orchestrator | Collector → Validator → Loader → Transformer | 동기 순차 호출 | Python 함수 호출 |
| Collector | External APIs | 동기 HTTP | REST API (HTTPS) |
| Loader | Snowflake raw_data | 동기 | Snowflake Connector (SQL) |
| Transformer | Snowflake Star Schema | 동기 | Snowflake SQL (CTAS) |
| Mart Manager | Snowflake Mart | 동기 | Snowflake SQL (CREATE VIEW) |
| Power BI | Snowflake Mart View | 동기 | Snowflake ODBC Connector |
| Pipeline Logger | 콘솔/파일 | 동기 | Python logging (실시간) |
| Pipeline Logger | Snowflake (execution_log, quality_log) | 동기 | INSERT (배치 완료 후 요약 적재) |
...
| 데이터 웨어하우스 | Snowflake | PostgreSQL, BigQuery | 클라우드 DW 업계 표준, 무료 트라이얼 비용 없이 학습 가능, Star Schema + Mart View 네이티브 지원 |
| 데이터 모델링 | Star Schema (Fact/Dim) | 3NF, Data Vault | 분석 최적화 표준 패턴, BI 도구와 궁합 우수, 초중급에서 학습 가치 가장 높은 패턴 |
| 시각화 | Power BI Desktop | Tableau, Metabase, Superset | Snowflake 네이티브 커넥터, 무료 Desktop 버전 사용 가능, 대시보드 업계 표준 |
...
> 배포 구성은 실무/외부 서비스 프로젝트에서 활성화됩니다. 본 프로젝트는 학습 목적으로, 로컬 실행 환경만 사용합니다.
...
| 설정 관리 | .env 파일 + python-dotenv, 종목 목록 등은 config.yaml | 섹션 1 제약사항 (키 관리) |
...
**상태**: 채택
**맥락**: Snowflake에 데이터를 적재하는 방식으로 ETL과 ELT 중 선택 필요
**결정**: ELT — raw 데이터를 Snowflake에 먼저 적재하고, Snowflake SQL로 Star Schema 변환
**근거**: Snowflake 컴퓨팅 파워를 활용한 SQL 기반 변환이 효율적, Python 수집 코드와 SQL 변환 코드의 역할 분리가 명확, 섹션 2 기존 파이프라인 흐름과 일치
...
**상태**: 채택
**맥락**: Snowflake raw_data 적재 시 기존 데이터 처리 방식 (Full Refresh vs Incremental)
**결정**: Full Refresh (TRUNCATE + INSERT)
**근거**: 학습용 소규모 데이터(수만 건)로 전체 재적재 부담 미미, 중복 체크 로직 불필요로 코드 단순화, 디버깅 시 전체 데이터 상태 일관성 보장
**결과**: 매 실행 시 동일한 결과 보장, Snowflake XS warehouse 크레딧 사용량 미미

---

## 11. 데이터 모델

> 섹션 3(기초 Star Schema)을 기반으로, Snowflake 타입으로 확장하고 운영 엔티티를 추가한다.
...
| high | NUMBER(18,4) | | 고가 |
| low | NUMBER(18,4) | | 저가 |
| close | NUMBER(18,4) | | 종가 |
| adj_close | NUMBER(18,4) | | 수정 종가 |
| volume | NUMBER(38,0) | | 거래량 |
| source | VARCHAR(20) | NOT NULL | 데이터 소스 (yfinance/KRX) |
| collected_at | TIMESTAMP_NTZ | NOT NULL, DEFAULT CURRENT_TIMESTAMP | 수집 시각 |

##### raw_economic_indicator

| 필드 | 타입 | 제약 | 설명 |
|---|---|---|---|
| indicator_code | VARCHAR(50) | NOT NULL | 지표 코드 (예: FEDFUNDS) |
| date | DATE | NOT NULL | 기준일 |
| value | NUMBER(18,6) | | 지표 값 |
| source | VARCHAR(20) | NOT NULL | 데이터 소스 (FRED) |
| collected_at | TIMESTAMP_NTZ | NOT NULL, DEFAULT CURRENT_TIMESTAMP | 수집 시각 |
...
| symbol | VARCHAR(20) | NOT NULL | 종목 코드 |
| date | DATE | NOT NULL | 기준일 |
| indicator_type | VARCHAR(20) | NOT NULL | 지표 유형 (RSI/MACD 등) |
| value | NUMBER(18,6) | | 지표 값 |
| source | VARCHAR(20) | NOT NULL | 데이터 소스 (Alpha Vantage) |
| collected_at | TIMESTAMP_NTZ | NOT NULL, DEFAULT CURRENT_TIMESTAMP | 수집 시각 |
...
| 필드 | 타입 | 제약 | 설명 |
|---|---|---|---|
| date_key | NUMBER(8,0) | FK → dim_date, NOT NULL | 날짜 키 (YYYYMMDD) |
| symbol_key | NUMBER(38,0) | FK → dim_symbol, NOT NULL | 종목 키 |
| open_price | NUMBER(18,4) | | 시가 |
| high_price | NUMBER(18,4) | | 고가 |
| low_price | NUMBER(18,4) | | 저가 |
| close_price | NUMBER(18,4) | | 종가 |
| adj_close | NUMBER(18,4) | | 수정 종가 |
| volume | NUMBER(38,0) | | 거래량 |
| loaded_at | TIMESTAMP_NTZ | NOT NULL, DEFAULT CURRENT_TIMESTAMP | 적재 시각 |

##### fact_economic_indicator

| 필드 | 타입 | 제약 | 설명 |
|---|---|---|---|
| date_key | NUMBER(8,0) | FK → dim_date, NOT NULL | 날짜 키 |
| indicator_key | NUMBER(38,0) | FK → dim_indicator, NOT NULL | 지표 키 |
| value | NUMBER(18,6) | NOT NULL | 지표 값 |
| loaded_at | TIMESTAMP_NTZ | NOT NULL, DEFAULT CURRENT_TIMESTAMP | 적재 시각 |
...
| 필드 | 타입 | 제약 | 설명 |
|---|---|---|---|
| date_key | NUMBER(8,0) | PK | 고유 키 (YYYYMMDD) |
| full_date | DATE | NOT NULL, UNIQUE | 전체 날짜 |
| year | NUMBER(4,0) | NOT NULL | 연도 |
| quarter | NUMBER(1,0) | NOT NULL | 분기 |
| month | NUMBER(2,0) | NOT NULL | 월 |
...
| 필드 | 타입 | 제약 | 설명 |
|---|---|---|---|
| symbol_key | NUMBER(38,0) | PK, AUTOINCREMENT | 고유 키 |
| ticker | VARCHAR(20) | NOT NULL, UNIQUE | 종목 코드 |
| company_name | VARCHAR(200) | | 회사명 |
| sector | VARCHAR(100) | | 섹터 |
| market | VARCHAR(10) | NOT NULL | 시장 (US/KR) |

##### dim_indicator

| 필드 | 타입 | 제약 | 설명 |
|---|---|---|---|
| indicator_key | NUMBER(38,0) | PK, AUTOINCREMENT | 고유 키 |
| indicator_code | VARCHAR(50) | NOT NULL, UNIQUE | 지표 코드 |
| indicator_name | VARCHAR(200) | NOT NULL | 지표명 |
| source | VARCHAR(20) | NOT NULL | 출처 (FRED/Alpha Vantage) |
| unit | VARCHAR(20) | | 단위 (%, 포인트 등) |
...
| 필드 | 타입 | 제약 | 설명 |
|---|---|---|---|
| log_id | NUMBER(38,0) | PK, AUTOINCREMENT | 고유 ID |
| execution_id | VARCHAR(36) | NOT NULL | 실행 ID (UUID) |
| stage | VARCHAR(20) | NOT NULL | 단계 (collect/validate/load/transform) |
| status | VARCHAR(10) | NOT NULL | 상태 (success/failure/warning) |
| record_count | NUMBER(38,0) | | 처리 건수 |
| error_message | VARCHAR(2000) | | 에러 메시지 |
| started_at | TIMESTAMP_NTZ | NOT NULL | 시작 시각 |
| ended_at | TIMESTAMP_NTZ | | 종료 시각 |

##### data_quality_log

| 필드 | 타입 | 제약 | 설명 |
|---|---|---|---|
| check_id | NUMBER(38,0) | PK, AUTOINCREMENT | 고유 ID |
| execution_id | VARCHAR(36) | NOT NULL | 실행 ID (pipeline_execution_log 연계) |
| check_type | VARCHAR(20) | NOT NULL | 검증 유형 (missing/outlier/duplicate) |
| target_table | VARCHAR(100) | NOT NULL | 대상 테이블 |
| target_field | VARCHAR(100) | | 대상 필드 |
| result | VARCHAR(10) | NOT NULL | 결과 (pass/warn/fail) |
| detail | VARCHAR(2000) | | 상세 내용 |
| checked_at | TIMESTAMP_NTZ | NOT NULL, DEFAULT CURRENT_TIMESTAMP | 검증 시각 |
...
| IR-002 | 날짜-종목 복합 유니크 | fact_daily_price (date_key + symbol_key) | 적재 시 |
| IR-003 | 날짜-지표 복합 유니크 | fact_economic_indicator (date_key + indicator_key) | 적재 시 |
| IR-004 | FK 참조 무결성 | fact → dim 테이블 모든 FK | 변환 시 |
...
|---|---|---|---|---|
| raw_* 테이블 | 배치 수집 시 | Full Refresh (매 실행) | 전체 재적재 (API가 전체 이력 제공, 별도 보관 정책 불필요) | TRUNCATE + INSERT |
| fact_* 테이블 | Star Schema 변환 시 | CTAS 재생성 (매 실행) | 영구 보관 | CTAS로 재생성 |
| dim_* 테이블 | 초기 세팅 + 변환 시 | 신규 종목/지표 추가 시 | 영구 보관 | 삭제 없음 (누적) |
| mart_* View | View 생성 시 | 조회 시 (View는 기반 테이블을 조회 시점에 읽음) | — | View이므로 별도 정책 불필요 |
| pipeline_execution_log | 파이프라인 실행 시 | 매 실행마다 INSERT | 영구 보관 | 삭제 없음 (누적) |
| data_quality_log | 품질 검증 시 | 매 실행마다 INSERT | 영구 보관 | 삭제 없음 (누적) |

### 11-5. 마이그레이션/진화 전략

> 학습 목적 프로젝트로 정식 마이그레이션 도구(Alembic, Flyway 등)는 비활성합니다.
> 스키마 변경 시 Snowflake SQL 스크립트를 직접 실행하여 관리합니다.
...
**워크플로우:**
```
자연어 질문 → 입력 검증 → 스키마 컨텍스트 조립 → LLM (SQL 생성) → SQL 검증 → Snowflake 실행 → LLM (결과 해석) → 응답
                                                      ↓ (실패 시)
                                                 폴백 처리
```
...
|---|---|---|---|---|---|
| AI-IO-001 | Text-to-SQL 변환 | 자연어 질문 (string, max 500자) | Snowflake SQL 쿼리 (string) | 빈 문자열 체크, 길이 제한 | SQL 파싱 검증, SELECT만 허용 (DML/DDL 차단) |
...
| 항목 | 내용 |
|---|---|
| 목적 | 자연어 금융 데이터 질문을 Snowflake SQL로 변환 |
| 시스템 프롬프트 | Snowflake SQL 전문가 역할. Star Schema 구조(Fact 2, Dim 3, Mart 3)를 이해하고, SELECT 문만 생성. 한국어/영어 질문 모두 처리. DML/DDL 절대 생성 금지 |
| 사용자 프롬프트 템플릿 | `스키마:\n{schema_info}\n\n질문: {user_question}\n\n위 스키마를 기반으로 Snowflake SQL을 생성하세요.` |
| 출력 형식 | SQL (코드 블록) |
| 예상 토큰 | 입력: ~800 / 출력: ~200 |
| 온도 | 0.0 (결정적 SQL 생성) |
...
| 잘못된 SQL 생성 | Snowflake 실행 에러 | 에러 메시지 포함하여 1회 재생성 → 실패 시 에러 반환 | "SQL 생성에 실패했습니다. 질문을 더 구체적으로 바꿔보세요." |
...
|---|---|---|---|
| SQL 실행 성공률 | 생성 SQL의 Snowflake 실행 성공/실패 비율 | > 80% | 수동 (학습 시) |
...
│   ├── loaders/                     # Snowflake 적재
│   │   ├── __init__.py
│   │   └── snowflake_loader.py      # raw_data Full Refresh 적재
...
│   ├── utils/                       # 공통 유틸리티
│   │   ├── __init__.py
│   │   ├── logger.py                # 구조화된 로깅 + Snowflake 로그 적재
│   │   └── config.py                # 설정 로딩 (.env + yaml)
│   └── main.py                      # 파이프라인 단일 진입점
├── sql/
│   ├── setup.sql                    # Snowflake 스키마 초기화 (전체 테이블)
│   ├── mart_views.sql               # Mart View 정의 (performance/risk/macro)
│   └── seed_dimensions.sql          # Dimension 초기 데이터 (날짜, 종목, 지표)
├── tests/                           # 단위 테스트 (선택 — 학습 시 작성)
├── docs/
│   └── data_dictionary.md           # 데이터 사전 (FR-011)
├── config/
│   └── symbols.yaml                 # 종목 목록, 지표 목록
├── .env.example
├── .gitignore
├── requirements.txt
└── README.md
...
| 변수 | 기본값 | 설명 | 필수 |
|---|---|---|---|
| SNOWFLAKE_ACCOUNT | — | Snowflake 계정 식별자 | Y |
| SNOWFLAKE_USER | — | Snowflake 사용자명 | Y |
| SNOWFLAKE_PASSWORD | — | Snowflake 비밀번호 | Y |
| SNOWFLAKE_DATABASE | FINANCE_DB | 데이터베이스명 | Y |
| SNOWFLAKE_WAREHOUSE | XS_WH | 웨어하우스명 | Y |
| SNOWFLAKE_SCHEMA | RAW_DATA | raw_data 스키마명 | Y |
...
| 범위 | 포함 FR | 설명 |
|---|---|---|
| **MVP** | FR-001,002,004,005,006,007,009,010,011 | 데이터 수집 → 적재 → 변환 → 시각화 + 품질검증/로깅/문서 |
