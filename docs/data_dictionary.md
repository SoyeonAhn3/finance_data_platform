...
| 유니버스 | **S&P 500 + Nasdaq-100** (ETF 보유종목 IVV+QQQ에서 매 실행 수집) |
...
Snowflake와 다른, 이 프로젝트에서 지켜야 하는 BigQuery 규약입니다.
...
| 기획서(Snowflake) | BigQuery | 비고 |
|---|---|---|
| `VARCHAR(n)` | `STRING` | 길이 제한 없음 |
| `NUMBER(18,4)`, `NUMBER(18,6)` | `NUMERIC` | precision 38 / scale 9 이내 |
| `NUMBER(38,0)` (volume) | `INT64` | 거래량은 INT64 범위 충분 |
| `NUMBER(8,0)` (date_key) | `INT64` | YYYYMMDD 8자리 정수 |
| `TIMESTAMP_NTZ` | `TIMESTAMP` | UTC 기준. `DEFAULT CURRENT_TIMESTAMP()` |
| `BOOLEAN` | `BOOL` | |
...
ETF(IVV+QQQ) 보유종목 → yfinance/FRED →  품질체크 →  BigQuery raw  →  dim/fact 재생성  →  분석용 View → Power BI
...
symbol  | date       | close | volume   | source   | collected_at
 aapl   | 2026-01-02 | 185.2 | 50000000 | yfinance | 09:05    ← 소문자+공백
AAPL    | 2026-01-02 | 185.2 | 50000000 | yfinance | 09:00    ← 정규화 후 위와 중복
XYZ     | 2026-01-02 |  10.0 | 100      | yfinance | 09:00    ← 명단(dim_symbol)에 없음
(null)  | 2026-01-05 |  50.0 | 999      | yfinance | 09:00    ← symbol 결측(raw는 허용)
MSFT    | 2026-01-03 |  -5.0 | 30000000 | yfinance | 09:00    ← 음수 종가(이상치)
```
...
- **설명**: S&P 500 + Nasdaq-100 구성종목 명단 (ETF 보유종목에서 매 실행 수집). **`dim_symbol`의 소스.**
- **소스**: ETF 보유종목 CSV — **IVV**(iShares, S&P500) + **QQQ**(Invesco, Nasdaq100)
- **처리**: 없음 (원본 보존). 수집 실패 시 직전 성공 목록 유지(캐시 폴백 — [8장](#8-유니버스--symbolsyaml))
...
- **설명**: yfinance에서 수집한 미국 주식 일별 OHLCV 원본
- **소스**: yfinance API (수집 대상 = `raw_universe`의 ticker 목록)
...
| volume | INT64 | | 거래량 |
| source | STRING | NOT NULL | 데이터 소스 (`yfinance`) |
| collected_at | TIMESTAMP | NOT NULL, DEFAULT `CURRENT_TIMESTAMP()` | 수집 시각 (UTC) |
...
SELECT
  FARM_FINGERPRINT(UPPER(TRIM(ticker))) AS symbol_key,
...
GROUP BY 1, 2;      -- ticker 기준 중복 제거 (두 지수에 걸친 종목 합침)
```
...
- **처리**: `WHERE indicator_code IS NOT NULL` → `FARM_FINGERPRINT(indicator_code)`로 대리키 생성
...
SELECT
  CAST(FORMAT_DATE('%Y%m%d', r.date) AS INT64) AS date_key,   -- 타입변환
  d.symbol_key,                                                -- 키 매핑(JOIN으로 획득)
...
FROM finance_db.raw_daily_price r
JOIN finance_db.dim_symbol d                                   -- 무결성 필터(inner join, dim=명단)
  ON d.ticker = UPPER(TRIM(r.symbol))                          -- 정규화
QUALIFY ROW_NUMBER() OVER (                                    -- 중복 제거
  PARTITION BY UPPER(TRIM(r.symbol)), r.date
...
a1b1... | run-0706     | universe | success | 517          | 08:59:00
a1b2... | run-0706     | collect  | success | 651420       | 09:00:00
a1b3... | run-0706     | load     | success | 651418       | 09:02:00
```
...
|---|---|---|---|---|
| **ETF 보유종목 (IVV+QQQ)** | S&P500+Nasdaq100 구성종목·회사명·섹터 | `raw_universe` | `dim_symbol` | ✅ |
| **yfinance** | 미국 주식 일별 OHLCV | `raw_daily_price` | `fact_daily_price` | ✅ |
...
### 일간 수익률 (mart_performance)

```sql
close_price / LAG(close_price) OVER (
  PARTITION BY symbol_key ORDER BY date_key
) - 1  AS daily_return
```
전일 종가 대비 오늘 종가의 변화율. `LAG`는 "이전 행의 값"을 가져오는 윈도우 함수.
...
```
일간 수익률의 표준편차 — 값이 클수록 가격 변동이 심함(=리스크 큼).

### 금리-주가 상관 (mart_macro)

```sql
CORR(stock_return, indicator_value)  AS correlation
```
날짜를 맞춘 뒤 주가 수익률과 지표 값의 상관계수(-1 ~ +1). 금리와 주가가 같이 움직이는지 반대인지 측정.
...
분석 대상은 **S&P 500 + Nasdaq-100**입니다. 종목을 손으로 나열하지 않고, **구성종목 목록을 매 실행마다 ETF 보유종목에서 받아옵니다.**

| 지수 | ETF | 운용사 | 신뢰 근거 |
|---|---|---|---|
| S&P 500 | **IVV** | BlackRock (iShares) | 운용사가 매일 공시하는 실제 보유 명세 |
| Nasdaq-100 | **QQQ** | Invesco | 동일 |

- Wikipedia(커뮤니티 편집, 비공식) 대신 **운용사 공시**를 씀 → 권위 있고 무료·키 불필요, 섹터(GICS)도 포함.
- 소스는 교체 가능(pluggable): `etf_holdings`(기본) / `api`(FMP·Finnhub) / `wikipedia`.
- **캐시 폴백**: 수집 실패 시 `raw_universe`를 직전 성공 목록으로 유지(TRUNCATE 안 함). 지수 멤버십은 분기마다 바뀌므로 하루 캐시는 문제없음.
- 규모: 두 지수 합쳐 ~510–530 유니크 종목(겹침 많음) → `fact_daily_price` ~65만 행(5년). BigQuery 무료티어 여유.
...
- 승인 종목인데 yfinance가 0건 반환(상폐 직후 등) → `data_quality_log`에 `completeness` 경고 (IR-008, FR-009)
